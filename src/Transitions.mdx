...
The default behavior is `jump`, which provides instant repositioning without animation.
...
By default, tooltips jump (fade out/in) between targets. To enable smooth move transitions:

```tsx
<TipMagicProvider options={{ transitionBehavior: 'move' }}>
...
Or enable move behavior per-element:

```tsx
<button data-tip="I will move smoothly!" data-tip-move>
  Move
</button>
```
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <code>moveTransitionDuration</code>
      </td>
      <td>number</td>
      <td>
        <code>200</code>
    ...
    <tr>
      <td>
        <code>respectReducedMotion</code>
      </td>
      <td>boolean</td>
      <td>
        <code>true</code>
...
<TipMagicProvider
  options={{
    transitionBehavior: 'move', // Enable smooth move transitions
...
<button data-tip="Smooth slide" data-tip-move>
  Move
</button>;
...
<button data-tip="Instant appear" data-tip-jump>
  Jump
</button>;
...
<nav>
  <a data-tip="Dashboard" data-tip-move data-tip-group="sidebar">
    Dashboard
  </a>
  <a data-tip="Analytics" data-tip-move data-tip-group="sidebar">
    Analytics
  </a>
  <a data-tip="Settings" data-tip-move data-tip-group="sidebar">
    Settings
  </a>
</nav>;
...
<header>
  <button data-tip="Search" data-tip-move data-tip-group="toolbar">
    Search
  </button>
  <button data-tip="Notifications" data-tip-move data-tip-group="toolbar">
    Notifications
  </button>
  <button data-tip="Profile" data-tip-move data-tip-group="toolbar">
    Profile
  </button>
</header>;
```
...
The library respects `prefers-reduced-motion` by default, for tooltip transitions and for
the tour panel's own animations:

```css
@media (prefers-reduced-motion: reduce) {
  .tip-magic-tooltip:not([data-tip-magic-motion='always']) {
    transition: none;
  }
}
```

`respectReducedMotion: false` in provider options opts back out: the provider then sets
`data-tip-magic-motion="always"` on the tooltip, which the guard above excludes, and
animations play regardless of the user's preference.
...
<div className="toolbar">
  <button data-tip="Bold; ⌘B" data-tip-move>
    B
  </button>
  <button data-tip="Italic; ⌘I" data-tip-move>
    I
  </button>
  <button data-tip="Underline; ⌘U" data-tip-move>
    U
  </button>
</div>
```
